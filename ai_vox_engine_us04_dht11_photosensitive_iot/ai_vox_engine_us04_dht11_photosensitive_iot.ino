...
static uint32_t s_last_sensor_read_time = 0;
...
int photosensitive_iot_read_value = 0;
...
void InitIot() {
  ...
  std::vector<ai_vox::iot::Property> photosensitive_sensor_iot_properties(
      {{"illuminance", "当前光敏值", ai_vox::iot::ValueType::kNumber},
       {"day_night", "当前昼夜状态(白天/夜晚)", ai_vox::iot::ValueType::kString}});
  ...
  g_photosensitive_sensor_iot_entity = std::make_shared<ai_vox::iot::Entity>("PhotosensitiveSensor",
                                                                             "光敏传感器",
                                                                             std::move(photosensitive_sensor_iot_properties),
                                                                             std::move(photosensitive_sensor_iot_functions));
...
void loop() {
  ...
  if (current_time - s_last_sensor_read_time >= kSensorReadInterval) {
    s_last_sensor_read_time = current_time;
    ...
    std::string light_status = photosensitive_iot_read_value > 500 ? "白天" : "晚上";
    g_photosensitive_sensor_iot_entity->UpdateState("illuminance", photosensitive_iot_read_value);
    g_photosensitive_sensor_iot_entity->UpdateState("day_night", light_status);
  }
